...
def select_costs(user_id, date_from=datetime.datetime(1, 1, 1), date_to=datetime.date.today(), category_id=None):  # выбирает расходы за определенный промежуток времени и/или по категории
    with sqlite3.connect('DataBase/db.db') as db:
        cursor = db.cursor()
        if category_id:
...
def select_income(user_id, date_from=datetime.datetime(1, 1, 1), date_to=datetime.date.today(), category_id=None):  # выбирает доходы за определенный промежуток времени и/или по категории
    with sqlite3.connect('DataBase/db.db') as db:
        cursor = db.cursor()
        if category_id:
